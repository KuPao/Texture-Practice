...
void Tri_Mesh::Render_SolidWireframe2()
{
	FIter f_it;
	FVIter	fv_it;
...
void Tri_Mesh::Render_Face(FIter tmpF)
{
	glDisable(GL_CULL_FACE);
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	glPushAttrib(GL_LIGHTING_BIT);
	...
	FVIter fv_it;
	glColor3f(0, 0, 1);
	for (fv_it = fv_iter(tmpF.handle()); fv_it; ++fv_it)
	{
		glNormal3dv(normal(fv_it.handle()).data());
		glVertex3dv(point(fv_it.handle()).data());
	}
...
void pChosen(OMT::FVIter tmp, Tri_Mesh *mesh)
{
	...
	glBegin(GL_POINTS);
	glColor3f(1, 0, 0);
	glVertex3dv(mesh->point(tmp.handle()).data());
	glEnd();
	glEnable(GL_LIGHTING);
	glDisable(GL_POLYGON_OFFSET_FILL);
}
...
void fChosen(OMT::FIter tmpF, Tri_Mesh * mesh)
{
	...
	glDisable(GL_LIGHTING);
	glEnable(GL_DEPTH_TEST);
	glPolygonOffset(1.5, 2.0);
	glBegin(GL_TRIANGLES);
	glColor3f(0, 0, 1);
	for (fv_it = mesh->fv_iter(tmpF); fv_it; ++fv_it)
	{
		glVertex3dv(mesh->point(fv_it.handle()).data());
	}
	glEnd();
